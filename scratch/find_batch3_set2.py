    ...
    with open(os.path.join(dir_path, file), encoding='utf-8-sig') as f:
        ...
        for i, row in enumerate(r):
            ...
            if len(row) > exp_idx:
                ...
                if 'thứ tự' in exp_clean.lower() or 'cú pháp' in exp_clean.lower() or 'logic' in exp_clean.lower() or 'n của ' in exp_clean.lower() or leakage_str.lower() in exp_clean.lower():
                    out.append(f'{file}-{i}: {exp}')
open(r'd:\pj\xx\ai-gen-quiz\scratch\batch3_set2_issues.txt', 'w', encoding='utf-8').write('\n'.join(out))
