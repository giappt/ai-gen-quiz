...
fixes = {}
for i in range(1, 4):
    with open(rf'd:\pj\xx\ai-gen-quiz\scratch\fixes_{i}.json', 'r', encoding='utf-8') as f:
...
for file_name, updates in files_to_update.items():
    ...
    for idx, row in enumerate(reader):
        # Fix malformed rows before writing
        if None in row:
            extra = row[None]
            if not row.get("Explanation") and extra:
                row["Explanation"] = extra[-1]
            del row[None]
            
        if idx in updates:
            fix_data = updates[idx]
